...
# 11. rosrun tf static_transform_publisher 0 0 0 0 0 0 1 reference/left_hand_camera left_hand_camera 10
# KEEP 11 RUNNING to publish transform
# How to Use:
# 1. python SmoothieMaster.py
# 2. Prompt user to enter a recipe
# 3. 

"""
FOR ASIMOV:
- camera feeds are switched, when you ask for image of left_hand_camera, you will see perspective from right _arm
- BUT frames are accurate
"""

# """
# Terminal Command List 12/11/19 
# 3. In ssh terminal (3): rosrun baxter_interface joint_trajectory_action_server.py
# 4. In ssh terminal (4): roslaunch baxter_moveit_config baxter_grippers.launch
# 1. In ssh terminal (1): roslaunch realsense2_camera rs_camera.launch filters:=pointcloud
# 2. In ssh terminal (2): roslaunch lab4_cam ar_track.launch / roslaunch lab4_cam ar_track_realsense.launch
# rosrun rviz rviz
# check for ar tags, if not there run static transform publisher
# when ready: run smoothiemaster
# """

class SmoothieMaster(object):

	#1. Launch realsense camera
	#2. Launch lab4_cam ar_track.launch for Baxter
	#3. Run SmoothieMaster.py
	#4. When prompted, exit lab4_cam ar_track.launch (Ctrl+C) 
	#   and enter the lab4_cam ar_track_realsense.launch
	#5. Press enter on SmoothieMaster.py window

	def __init__(self, gripper_arm="right"):
		print("Initializing SmoothieMaster...")
		rospy.init_node('SmoothieMaster', anonymous=True)
		self.set_left_arm_camera()
		self.gripper_arm = gripper_arm

		# All AR tags we will need
		self.ar_marker_tag = "ar_marker_4"
		self.blender_ar = "ar_marker_9"
		self.lid_ar = "ar_marker_0"
		#finding the transformation from the ar_tag on the able to the base of the robot
		
		#Launch roslaunch lab4_cam ar_track.launch, ctrl c when done
		self.ar_to_base = self.find_transform("base", self.ar_marker_tag)
		self.g_ar_to_base = self.msg_to_se3(self.ar_to_base)

		self.ar_to_blender = self.find_transform("base", self.blender_ar)
		self.reset_transform = self.ar_to_blender

		self.ar_to_lid = self.find_transform("base", self.lid_ar)

		
		raw_input("Press enter when exited Baxter camera")

		#Launch roslaunch lab4_cam ar_track_realsense.launch, ctrl c when done
		#finding the transformation from the real sense camera to the ar_tag on the table
		self.realsensecamera_to_ar = self.find_transform(self.ar_marker_tag,"camera_depth_optical_frame")
	...
	def main(self):
		...
		9. close the gd lid
		"""
		#Left Hand Camera Position from base to left_hand_camera
		#- Translation: [0.601, 0.233, 0.360]
		# Rotation: in Quaternion [0.788, 0.507, -0.045, 0.346]
  #           in RPY (radian) [2.558, 0.435, 1.011]
  #           in RPY (degree) [146.589, 24.903, 57.924]
		...
		for fruit_desired in order:
			...
			print("Fruit detected: ", available_fruit)
			...
			for fruit in available_fruit:
				if fruit.get_fruit_type() == fruit_desired:
					target_fruit = fruit
					break
			pts = np.asarray(target_fruit.get_points()[::100])

			x = pts[:,0]
			y = pts[:,1]
			centro = target_fruit.get_centroid()
			print("Grabbing fruit: ", target_fruit.get_fruit_type())
			print("centroid: ",centro)
			# fig = plt.figure()
			# ax = fig.add_subplot(111, projection='3d')
			# ax.scatter(x,y, marker='.',label="pointcloud")
			# ax.scatter(centro[0],centro[1],0,marker='x',color="red",label="centroid")
			# ax.legend()
			# plt.show()			
			...
			fruit_picker = FruitPicker(target_fruit, self.blender_ar, self.gripper_arm)
			fruit_picker.publish_fruit_pose(target_fruit)
			print("exited publish fruit pose")
			fruit_picker.reset_arm(self.reset_transform)

			fruit_picker.move_to_fruit()
			fruit_picker.grab_fruit()

			fruit_picker.reset_arm(self.reset_transform)
			fruit_picker.move_to_blender(self.ar_to_blender)



		# close lid
		lid_closer = FruitPicker(None, self.blender_ar, self.gripper_arm)
		lid_closer.reset_arm(self.reset_transform)
		lid_closer.move_to_lid(self.ar_to_lid)
		lid_closer.grab_lid(self.ar_to_lid)

		lid_closer.reset_arm(self.reset_transform)
		lid_closer.move_to_blender(self.ar_to_blender)
...
# OBSOLETE -- METHOD MOVED TO FruitClassifier.py
	...
	def add_transform_attributes(self,fruit, z_default=-0.13):
		...
		cent_trans[2] = z_default
		print("cent trans: ",cent_trans)
		pc1 = fruit.get_pc1()
		pc1_homo = np.array([pc1[0],pc1[1],0.0,1])
		pc1_trans = np.dot(self.g_ar_to_base,pc1_homo)

		pc2 = fruit.get_pc2()
		pc2_homo = np.array([pc2[0],pc2[1],0.0,1])
		...
		fruit.assignAttributes(cent_trans, pc1_trans, pc2_trans)

	def set_left_arm_camera(self):
		...
		joint_command = {
			'left_w0': 0.4866554049566475, 
			'left_w1': 0.7992039904883006, 
			'left_w2': -3.0583741958465436, 
			'left_e0': -1.6229516735827678, 
			'left_e1': 1.9945585194480093, 
			'left_s0': 0.5752427954570302, 
			'left_s1': -0.6250971710633061}
		rate = rospy.Rate(10.0)
		threshold = 0.1
		within_thresh = False
		delta = [0]*7
		while not within_thresh:
			left.set_joint_positions(joint_command)
			angle_dict = left.joint_angles()

			delta[0] = abs(joint_command['left_s0'] - angle_dict['left_s0'])
			delta[1] = abs(joint_command['left_s1'] - angle_dict['left_s1'])
			delta[2] = abs(joint_command['left_e0'] - angle_dict['left_e0'])
			delta[3] = abs(joint_command['left_e1'] - angle_dict['left_e1']) 
			delta[4] = abs(joint_command['left_w0'] - angle_dict['left_w0']) 
			delta[5] = abs(joint_command['left_w1'] - angle_dict['left_w1']) 
			delta[6] = abs(joint_command['left_w2'] - angle_dict['left_w2']) 

			within_thresh = all(map(lambda x: x < threshold, delta))
